...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn groth16_verification_opt(
			...
			let vk = VerifyingKeyFor::<ark_bls12_381::Bls12_381, BlsFr>::deserialize_uncompressed_unchecked(&vk[..]).map_err(|_| Error::<T>::Serialization)?;
			let vk = utils::serialize_argument(vk);

			let c = ark_ff::Fp::<ark_ff::MontBackend<ark_bls12_381::FrConfig, 4>,
		4>::deserialize_uncompressed_unchecked(&c[..]) 			.map_err(|_| Error::<T>::Serialization)?;
			let c = crate::utils::serialize_argument(c);
		...
		// 	scalar: Vec<u8>,
		// ) -> DispatchResult {
		// 	let base = <ArkScaleProjective<ed_on_bls12_381_bandersnatch::SWProjectiveOptimized> as
		// Decode>::decode( 		&mut base.as_slice(),
		// 	)
		// 	.unwrap()
		// 	.0;
		...
		// ) -> DispatchResult {
		// 	let base =
		// 		<ArkScaleProjective<ed_on_bls12_381_bandersnatch::EdwardsProjectiveOptimized> as
		// Decode>::decode( 			&mut base.as_slice(),
		// 		)
		// 		.unwrap()
		// 		.0;
		...
		// 	scalar: Vec<u8>,
		// ) -> DispatchResult {
		// 	let base = <ArkScaleProjective<ed_on_bls12_377::EdwardsProjectiveOptimized> as
		// Decode>::decode( 		&mut base.as_slice(),
		// 	)
		// 	.unwrap().0;
		// 	let scalar = <ArkScale<Vec<u64>> as Decode>::decode(&mut scalar.as_slice()).unwrap().0;
