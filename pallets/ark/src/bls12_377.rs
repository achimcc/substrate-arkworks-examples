...
pub fn msm_g1_opt(bases: &[G1AffineOpt], scalars: &[ScalarFieldFor<G1AffineOpt>]) {
	let _ = <sp_bls12_377::g1::Config as SWCurveConfig>::msm(bases, scalars);
}
...
pub fn mul_affine_g1_opt(base: &G1AffineOpt, scalar: &[u64]) {
	let _out = <sp_bls12_377::g1::Config as SWCurveConfig>::mul_affine(base, scalar);
}
...
pub fn mul_projective_g1_opt(base: &G1ProjectiveOpt, scalar: &[u64]) {
	let _out = <sp_bls12_377::g1::Config as SWCurveConfig>::mul_projective(base, scalar);
}
...
pub fn mul_affine_g2_opt(base: &G2AffineOpt, scalar: &[u64]) {
	let _out = <sp_bls12_377::g2::Config as SWCurveConfig>::mul_affine(base, scalar);
}
...
pub fn mul_projective_g2_opt(base: &G2ProjectiveOpt, scalar: &[u64]) {
	let _out = <sp_bls12_377::g2::Config as SWCurveConfig>::mul_projective(base, scalar);
}
