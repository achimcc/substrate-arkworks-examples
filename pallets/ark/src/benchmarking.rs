...
mod benchmarks {
	...
	fn bls12_381_groth16_verification() {
		...
		let proof =
			ProofFor::<ark_bls12_381::Bls12_381, ark_bls12_381::Fr>::deserialize_compressed_unchecked(PROOF_SERIALIZED)
				.unwrap();
		let proof = serialize_argument(proof);

		#[extrinsic_call]
		bls12_381_groth16_verification(RawOrigin::None, vk, c, proof);
	}
	...
	fn bls12_381_groth16_verification_opt() {
		...
		#[extrinsic_call]
		bls12_381_groth16_verification_opt(RawOrigin::None, vk, c, proof);
	}
	...
	fn bls12_381_pairing() {
		...
		#[extrinsic_call]
		bls12_381_pairing(RawOrigin::None, a, b);
	}
	...
	#[benchmark]
	fn bls12_381_msm_g1(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_bls12_381::G1Projective>(x);
	...
	#[benchmark]
	fn bls12_381_msm_g2(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_bls12_381::G2Projective>(x);
	...
	#[benchmark]
	fn bls12_381_mul_projective_g1(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args_projective::<ark_bls12_381::G1Projective>(x);

		#[extrinsic_call]
		bls12_381_mul_projective_g1(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	#[benchmark]
	fn bls12_381_mul_affine_g1(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_bls12_381::G1Affine>(x);

		#[extrinsic_call]
		bls12_381_mul_affine_g1(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	#[benchmark]
	fn bls12_381_mul_projective_g2(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args_projective::<ark_bls12_381::G2Projective>(x);

		#[extrinsic_call]
		bls12_381_mul_projective_g2(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	#[benchmark]
	fn bls12_381_mul_affine_g2(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_bls12_381::G2Affine>(x);

		#[extrinsic_call]
		bls12_381_mul_affine_g2(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	#[benchmark]
	fn bls12_377_msm_g1(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_bls12_377::G1Projective>(x);

		#[extrinsic_call]
		bls12_377_msm_g1(RawOrigin::None, bases.encode(), scalars.encode());
	}
	...
	fn bls12_377_msm_g2(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_bls12_377::G2Projective>(x);
	...
	#[benchmark]
	fn bls12_377_mul_projective_g1(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args_projective::<ark_bls12_377::G1Projective>(x);

		#[extrinsic_call]
		bls12_377_mul_projective_g1(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	#[benchmark]
	fn bls12_377_mul_affine_g1(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_bls12_377::G1Affine>(x);

		#[extrinsic_call]
		bls12_377_mul_affine_g1(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	fn bls12_377_mul_projective_g2(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args_projective::<ark_bls12_377::G2Projective>(x);

		#[extrinsic_call]
		bls12_377_mul_projective_g2(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	#[benchmark]
	fn bls12_377_mul_affine_g2(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_bls12_377::G2Affine>(x);

		#[extrinsic_call]
		bls12_377_mul_affine_g2(RawOrigin::None, base.encode(), scalar.encode());
	}
	...
	fn bw6_761_msm_g1(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_bw6_761::G1Projective>(x);
	...
	fn bw6_761_msm_g2(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_bw6_761::G2Projective>(x);
	...
	fn bw6_761_mul_projective_g1(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args_projective::<ark_bw6_761::G1Projective>(x);
	...
	fn bw6_761_mul_affine_g1(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_bw6_761::G1Affine>(x);
	...
	fn bw6_761_mul_projective_g2(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args_projective::<ark_bw6_761::G2Projective>(x);
	...
	fn bw6_761_mul_affine_g2(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_bw6_761::G2Affine>(x);
	...
	fn ed_on_bls12_381_bandersnatch_msm_sw(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_ed_on_bls12_381_bandersnatch::SWProjective>(x);
	...
	fn ed_on_bls12_381_bandersnatch_msm_te(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) =
			make_msm_args::<ark_ed_on_bls12_381_bandersnatch::EdwardsProjective>(x);
	...
	fn ed_on_bls12_381_bandersnatch_mul_projective_sw(
		...
		let (base, scalar) =
			make_scalar_args_projective::<ark_ed_on_bls12_381_bandersnatch::SWProjective>(x);
	...
	fn ed_on_bls12_381_bandersnatch_mul_affine_sw(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_ed_on_bls12_381_bandersnatch::SWAffine>(x);
	...
	fn ed_on_bls12_381_bandersnatch_mul_projective_te(
		...
		let (base, scalar) =
			make_scalar_args_projective::<ark_ed_on_bls12_381_bandersnatch::EdwardsProjective>(x);
	...
	fn ed_on_bls12_381_bandersnatch_mul_affine_te(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) = make_scalar_args::<ark_ed_on_bls12_381_bandersnatch::EdwardsAffine>(x);
	...
	fn ed_on_bls12_377_msm(x: Linear<MSM_LEN_MIN, MSM_LEN_MAX>) {
		let (bases, scalars) = make_msm_args::<ark_ed_on_bls12_377::EdwardsProjective>(x);
	...
	fn ed_on_bls12_377_mul_projective(x: Linear<SCALAR_WORDS_MIN, SCALAR_WORDS_MAX>) {
		let (base, scalar) =
			make_scalar_args_projective::<ark_ed_on_bls12_377::EdwardsProjective>(x);
