...
pub fn msm_te_opt(bases: &[EdwardsAffineOpt], scalars: &[ScalarFieldFor<EdwardsAffineOpt>]) {
	let _out =
		<sp_ed_on_bls12_381_bandersnatch::BandersnatchConfig as TECurveConfig>::msm(bases, scalars);
}
...
pub fn mul_affine_te_opt(base: &EdwardsAffineOpt, scalar: &[u64]) {
	let _out =
		<sp_ed_on_bls12_381_bandersnatch::EdwardsConfig as TECurveConfig>::mul_affine(base, scalar);
}
...
pub fn mul_projective_te_opt(base: &EdwardsProjectiveOpt, scalar: &[u64]) {
	let _out = <sp_ed_on_bls12_381_bandersnatch::EdwardsConfig as TECurveConfig>::mul_projective(
		base, scalar,
	);
}
